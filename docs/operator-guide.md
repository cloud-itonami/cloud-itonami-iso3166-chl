...
2. Register the client's intake: business type, target public function,
   prior filing history in Chile if any.
3. Run the advisor in read-only mode against ChileCompra (Mercado Público), Chile's central public-procurement platform.
...
- named referral relationship with Chilean-licensed counsel or a
...
a working referral relationship with Chilean-licensed counsel or a
registered agent for whatever licensed representation the law of
Chile requires for actual public-procurement filings.
