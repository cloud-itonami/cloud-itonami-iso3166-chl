# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Chile

## Classification

- Repository: `cloud-itonami-iso3166-chl`
- ISO 3166: `CHL` (Chile)
...
- Social impact: [:sme-regional-supplier-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Chilean
  public contract
...
- registration walkthrough for ChileCompra (Mercado Público), Chile's central public-procurement platform
- business/tax registration checklist: a RUT (Rol Único Tributario) from the SII (Servicio de Impuestos Internos) and, where relevant, a registration in the Registro de Proveedores del Estado; VAT (IVA) registration with the SII
- local-content / preferential-procurement navigation: no blanket local-content quota, but ChileCompra's evaluation criteria typically weight SME/regional supplier participation on smaller tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Chilean-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Chile).
