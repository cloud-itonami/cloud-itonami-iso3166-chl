(ns marketentry.store
  "SSoT for the CHL market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems CL" :portal "Mercado Público"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-cl-entity? true :has-cl-entity? true
             :requires-rut? true :rut-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CHL" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "Mercado Público"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-cl-entity? true :has-cl-entity? true
             :requires-rut? true :rut-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "Mercado Público"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "CHL" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "Mercado Público"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-cl-entity? true :has-cl-entity? false
             :requires-rut? true :rut-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CHL" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "Mercado Público"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-cl-entity? true :has-cl-entity? true
             :requires-rut? true :rut-verified? false
             :drafted? false :submitted? false
             :jurisdiction "CHL" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "Mercado Público"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-cl-entity? has-cl-entity?
                               requires-rut? rut-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-cl-entity?)  (assoc :engagement/requires-cl-entity? requires-cl-entity?)
    (some? has-cl-entity?)       (assoc :engagement/has-cl-entity? has-cl-entity?)
    (some? requires-rut?)    (assoc :engagement/requires-rut? requires-rut?)
    (some? rut-verified?)    (assoc :engagement/rut-verified? rut-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-cl-entity? :engagement/has-cl-entity?
   :engagement/requires-rut? :engagement/rut-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-cl-entity? (boolean (:engagement/requires-cl-entity? m))
     :has-cl-entity? (boolean (:engagement/has-cl-entity? m))
     :requires-rut? (boolean (:engagement/requires-rut? m))
     :rut-verified? (boolean (:engagement/rut-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-chl"})
