(ns marketentry.marketentryllm
  ...
  the CHL public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-cl-entity?=" (:has-cl-entity? e)
                        " rut-verified?=" (:rut-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-cl-entity? e))
                              (:has-cl-entity? e))
                          (or (not (:requires-rut? e))
                              (:rut-verified? e)))
                   0.9 0.3)}))
