    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(RUT/統一資格/Mercado Público登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- cl-entity-missing-violations
...
(defn- rut-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-rut? true`, INDEPENDENTLY check
  `:rut-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-rut? e))
                 (not (true? (:rut-verified? e))))
        [{:rule :rut-unverified
          :detail (str subject " はRUT確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (cl-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (rut-unverified-violations request st)
